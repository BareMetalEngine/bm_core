...
TEST(Array, CreatedArrayReportsAsEmpty)
{
	Array<int> a;
	EXPECT_TRUE(a.empty());
}

TEST(Array, CreatedArrayReportsWithZeroSize)
{
	Array<int> a;
	EXPECT_EQ(0, a.size());
}

TEST(Array, CreatedArrayReportsWithZeroCapacity)
{
	Array<int> a;
	EXPECT_EQ(0, a.capacity());
}

TEST(Array, CreatedArrayReportsAsFull)
{
	Array<int> a;
	EXPECT_TRUE(a.full());
}

TEST(Array, CreatedArrayHasNullDataPointer)
{
	Array<int> a;
	EXPECT_EQ(nullptr, a.data());
}

TEST(Array, CreateFromSizeResportsNotEmpty)
{
	Array<int> a(1);
	EXPECT_FALSE(a.empty());
}

TEST(Array, CreateFromSizeResportsSize)
{
	Array<int> a(1, 0);
	EXPECT_EQ(1, a.size());
}

TEST(Array, CreatedArrayHasNonNullData)
{
	Array<int> a(1, 0);
	EXPECT_NE(nullptr, a.data());
}

TEST(Array, CreatedArrayContainsElement)
{
	Array<int> a(1, 42);
	EXPECT_EQ(42, a[0]);
}

TEST(Array, CreateArrayFromArrayViewReportsSize)
{
	Array<int> src(2, 0);
	auto view = src.view();
...
TEST(Array, CreateArrayFromArrayViewDoestNotModifySourceView)
{
	Array<int> src(2, 0);
	src[0] = 69;
	src[1] = 420;
...
TEST(Array, CreateArrayFromArrayCopiesElements)
{
	Array<int> src(2, 0);
	src[0] = 69;
	src[1] = 420;
	...
	Array<int> dest(view);
	EXPECT_EQ(69, dest[0]);
	EXPECT_EQ(420, dest[1]);
}

TEST(Array, CreateArrayFromCArray)
{
	uint32_t data[3] = { 1,2,3 };

	Array<uint32_t> a(data);
	EXPECT_FALSE(a.empty());
}

TEST(Array, CreateArrayFromCArrayHasValidSize)
{
	uint32_t data[3] = { 1,2,3 };
...
TEST(Array, CreateArrayFromElementCopiesIt)
{
	{
		tests::SimpleTrackerScope tracking;

		tests::SimpleTracker elem;
		Array<tests::SimpleTracker> src(2, elem);

		EXPECT_EQ(3, tests::SimpleTracker::Population());
		EXPECT_EQ(1, tests::SimpleTracker::NumDefaultConstructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumValueConstructed);
		EXPECT_EQ(2, tests::SimpleTracker::NumCopyConstructed);
...
TEST(Array, CreateArrayFromTempElementCopiesIt)
{
	{
		tests::SimpleTrackerScope tracking;

		Array<tests::SimpleTracker> src(2, tests::SimpleTracker());

		EXPECT_EQ(2, tests::SimpleTracker::Population());
		EXPECT_EQ(1, tests::SimpleTracker::NumDefaultConstructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumValueConstructed);
		EXPECT_EQ(2, tests::SimpleTracker::NumCopyConstructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumCopyAssigned);
...
TEST(Array, CreateArrayFromArrayViewCopiesElements)
{
	{
		tests::SimpleTrackerScope tracking;
		...
		Array<tests::SimpleTracker> dest(srcView);

		EXPECT_EQ(4, tests::SimpleTracker::Population());
		EXPECT_EQ(1, tests::SimpleTracker::NumDefaultConstructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumValueConstructed);
		EXPECT_EQ(4, tests::SimpleTracker::NumCopyConstructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumCopyAssigned);
		EXPECT_EQ(1, tests::SimpleTracker::NumDestructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumMoveAssigned);
		EXPECT_EQ(0, tests::SimpleTracker::NumMoveConstructed);
...
TEST(Array, CreateArrayFromInitializationListMovesElements)
{
	{
		tests::SimpleTrackerScope tracking;

		Array<tests::SimpleTracker> dest = { tests::SimpleTracker() };

		// waiting for C++20...
#if 0
		EXPECT_EQ(1, tests::SimpleTracker::Population());
		EXPECT_EQ(1, tests::SimpleTracker::NumDefaultConstructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumValueConstructed);
...
TEST(Array, CreateArrayFromCArrayCopiesElements)
{
	{
		tests::SimpleTrackerScope tracking;

		tests::SimpleTracker data[2] = { 2, 3 };

		Array<tests::SimpleTracker> dest = data;

		// waiting for C++20...
		EXPECT_EQ(4, tests::SimpleTracker::Population());
		EXPECT_EQ(0, tests::SimpleTracker::NumDefaultConstructed);
		EXPECT_EQ(2, tests::SimpleTracker::NumValueConstructed);
		EXPECT_EQ(2, tests::SimpleTracker::NumCopyConstructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumCopyAssigned);
		EXPECT_EQ(0, tests::SimpleTracker::NumDestructed);
		EXPECT_EQ(0, tests::SimpleTracker::NumMoveAssigned);
...
TEST(Array, CreateDynamicBufferMakesCopy)
{
	...
	auto buf = Buffer::CreateFromCopy(MainPool(), a.bufferView());
...
TEST(Array, CreateInlinedBufferReferencesOriginal)
{
	...
	auto buf = Buffer::CreateExternal(a.bufferView(), [](void*) {});
	ASSERT_TRUE(buf != nullptr);
	ASSERT_EQ(a.dataSize(), buf.size());
