...
template< typename T >
static void PrintToString(TempString& txt, T val)
{
    IType::PrintToString(GetTypeObject<T>(), & val, txt);
}

void PrintToString(IFormatStream& f, const char* txt)
{
//    IType::PrintToString(GetTypeObject<T>(), &val, txt);
    f.append(txt, strlen(txt));
}
...
{
    StringView(txt).encode(EncodingType::CString, f);
    //f.appendCEscaped(txt, ARRAY_COUNT(txt) - 1, willBeSentInQuotes);
}

void PrintCString(IFormatStream& f, const char* txt, bool willBeSentInQuotes = false)
{
    StringView(txt).encode(EncodingType::CString, f);
    //f.appendCEscaped(txt, strlen(txt), willBeSentInQuotes);
}

TEST(RttiToString, String)
{
    TempString txt;
...
template< typename T >
static bool ParseFromString(StringView txt, T& val)
{
    return IType::ParseFromString(GetTypeObject<T>(), &val, txt);
}
...
TEST(RttiFromString, IntoStaticStringBuffer)
{
    ...
    EXPECT_FALSE(ParseFromString("Ala ma kota", buffer));
