...
class BM_CORE_MATH_API Vector3
{
...
public:
    ...
    INLINE Vector3 minScalar(float val) const;

    // maximum values
    INLINE Vector3 max(const Vector3& other) const;

    // maximum values
    INLINE Vector3 maxScalar(float val) const;

    // clamped to range
    INLINE Vector3 clamped(const Vector3& minV, const Vector3& maxV) const;

    // clamped to range
    INLINE Vector3 clampedScalar(float minV, float maxV) const;

    // clamp in place to range
    INLINE void clamp(const Vector3& minV, const Vector3& maxV);

    // clamp in place to range
    INLINE void clampScalar(float minV, float maxV);

    // lerp between this vector and target
    INLINE Vector3 lerp(const Vector3& target, float f) const;

    // lerp between this vector and target, per component weight
    INLINE Vector3 lerpPerComponent(const Vector3& target, const Vector3& f) const;
    ...
    INLINE Vector3 clampedLength(float length) const;
