...
class BM_CORE_MATH_API ExactPosition
{
...
public:
    ...
    INLINE float distanceFast(const Vector3& base) const; // even less accurate
    ...
    INLINE float squareDistanceFast(const Vector3& base) const; // even less accurate
...
private:
    INLINE Vector3 approx() const { return Vector3(x, y, z); }
};
