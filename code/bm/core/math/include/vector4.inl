...
INLINE Vector4 Vector4::minScalar(float val) const
{
    return Vector4(std::min(x, val), std::min(y, val), std::min(z, val), std::min(w, val));
}
...
INLINE Vector4 Vector4::maxScalar(float val) const
{
    return Vector4(std::max(x, val), std::max(y, val), std::max(z, val), std::max(w, val));
}

INLINE Vector4 Vector4::clamped(const Vector4& minV, const Vector4& maxV) const
{
    return Vector4(
        std::clamp(x, minV.x, maxV.x),
        std::clamp(y, minV.y, maxV.y),
        std::clamp(z, minV.z, maxV.z),
        std::clamp(w, minV.w, maxV.w));
}

INLINE Vector4 Vector4::clampedScalar(float minV, float maxV) const
{
    return Vector4(
        std::clamp(x, minV, maxV),
        std::clamp(y, minV, maxV),
        std::clamp(z, minV, maxV),
        std::clamp(w, minV, maxV));
}

INLINE void Vector4::clamp(const Vector4& minV, const Vector4& maxV)
{
    x = std::clamp(x, minV.x, maxV.x);
    y = std::clamp(y, minV.y, maxV.y);
    z = std::clamp(z, minV.z, maxV.z);
    w = std::clamp(w, minV.w, maxV.w);
}

INLINE void Vector4::clampScalar(float minV, float maxV)
{
    x = std::clamp(x, minV, maxV);
    y = std::clamp(y, minV, maxV);
    z = std::clamp(z, minV, maxV);
    w = std::clamp(w, minV, maxV);
}

INLINE Vector4 Vector4::lerp(const Vector4& target, float f) const
{
    return Vector4(
        x + (target.x - x) * f,
        y + (target.y - y) * f,
...
INLINE Vector4 Vector4::lerpPerComponent(const Vector4& target, const Vector4& f) const
{
    return Vector4(
        x + (target.x - x) * f.x,
        y + (target.y - y) * f.y,
...
INLINE void Vector4::snap(float grid)
{
    x = Snap(x, grid);
    y = Snap(y, grid);
    z = Snap(z, grid);
...
INLINE Vector4 Vector4::snapped(float grid) const
{
    return Vector4(Snap(x, grid), Snap(y, grid), Snap(z, grid), Snap(w, grid));
}
...
{
    ASSERT_EX(index >= 0 && index <= 3, "Invalid component index");
    return (&x)[index];
}
