...
{
    return std::sqrt(squareDistance(base));
}

INLINE float ExactPosition::distanceFast(const Vector3& base) const
{
    return std::sqrt(squareDistanceFast(base));
}
...
INLINE float ExactPosition::squareDistanceFast(const Vector3& base) const
{
