...
struct BM_CORE_MATH_API CameraSetup
{
    RTTI_DECLARE_NONVIRTUAL_CLASS(CameraSetup);
...
public:
    ...
    bool reverseProjection = false; // camera has the Z range inverted = 1.0f is near place and 0.0f is far plane
    ...
    // calculate the world to screen matrix for the camera's setup
    void calcWorldToScreen(Matrix& outWorldToScreen) const;
...
public:
    ...
    /// get the Z linearlization vector
    Vector4 linearZVector() const;
...
struct BM_CORE_MATH_API VisibilityBox
{
    SIMDQuad min;
    SIMDQuad max;
...
struct BM_CORE_MATH_API VisibilityFrustum
{
    ...
    // setup from position and inverse projection matrix
    void setupPerspective(const Vector3& position, const Matrix& worldToScreen);

    // setup from ortho camera
    void setupOrtho(const Vector3& position, const Matrix& screenToWorld);

    //--

    static const uint8_t TEST_OUTSIDE = 0;
    static const uint8_t TEST_INTERSECTS = 1;
    static const uint8_t TEST_INSIDE = 2;

    // are we inside a given frustum
    uint8_t testBox(const VisibilityBox& f) const;

    //--

    // extract planes for use in shaders
    void extractPlanes(Vector4* outPlanes) const;
