...
public:
    ...
    uint8_t r = 255, g = 255, b = 255, a = 255; // <- read right to left ot get the name, ABGR in this case
    ...
    // scale color, no SRGB conversion (inaccurate)
    Color scaleColorRaw(float scale) const;
    ...
    // interpolate color in sRGB space
    Color lerp(Color target, float frac) const;

    // interpolate color values directly
    Color lerpRaw(Color target, float frac) const;

    // per component minimum
    INLINE Color min(Color other) const;

    // per component maximum
    INLINE Color max(Color other) const;
    ...
    //! Convert to RGBA linear color values, this assumes what is true for all colors that they are in sRGB space
    //! Thus all output color components are converted via SRGB to Linear function
    Vector4 toVector() const;

    //! Convert to vector with simple conversion
    Vector4 toVectorRaw() const;

    //! Convert to style hex representation #FF00FF
    StringBuf toHexString(bool withAlpha = false) const;

    //---
        
    // calculate color luminance (sRGB space implied, luminance in linear space)
    float luminance() const;

    // calculate color luminance (raw values)
    float luminanceRaw() const;

    // get inverted color (sRGB space implied)
    Color inverted() const;

    // get inverted color (just raw values)
    Color invertedRaw() const;
    ...
    //! Initialize from linear color space vector, assumes color is stored in sRGB space (thus all incoming values are converted)
    static Color FromVector(const Vector4 &other);

    //! Initialize from a vector by means of simple conversion 
    static Color FromVectorRaw(const Vector4 &other);
...
// SRGB <-> Linear conversion tables
class BM_CORE_MATH_API ColorLookupTablesSRGB
{
public:
    float Uint8ToLinear[256]; // 0-255 sRGB value to linear 0-1 float
    float Uint8SrgbToLinear[256]; // 0-255 sRGB value to linear 0-1 float, basically x/255.0f
    float Uint16SrgbToLinear[65536]; // 0-65535 sRGB value to linear 0-1 float

    uint8_t Uint8LinearToSrgb[65537]; // 0.0f-65536.0f (for easier quantization) linear value into 8-bit sRGB
    uint16_t Uint16LinearToSrgb[65537]; // 0.0f-65536.0f (for easier quantization) linear value into 16-bit sRGB

public:
    ColorLookupTablesSRGB();

    static const ColorLookupTablesSRGB& GetInstance(); // global singleton
};
