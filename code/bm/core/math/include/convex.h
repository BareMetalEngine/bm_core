...
public:
    ...
    /// get raw vertices
    INLINE const Vertex* vertices() const { return valid() ? OffsetPtr<Vertex>(m_data.data(), header().vertexOffset) : nullptr; }

    /// get raw edges
    INLINE const ConvexEdge* edges() const { return valid() ? OffsetPtr<ConvexEdge>(m_data.data(), header().edgeOffset) : nullptr; }

    /// get raw planes
    INLINE const Plane* planes() const { return valid() ? OffsetPtr<Plane>(m_data.data(), header().planeOffset) : nullptr; }

    /// get raw faces
    INLINE const Face* faces() const { return valid() ? OffsetPtr<Face>(m_data.data(), header().faceOffset) : nullptr; }
