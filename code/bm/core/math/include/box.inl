...
INLINE Box::Box(const Vector3& start, const Vector3& end, float extents)
{
    min.x = std::min(start.x - extents, end.x - extents);
    min.y = std::min(start.y - extents, end.x - extents);
    min.z = std::min(start.z - extents, end.x - extents);
    max.x = std::max(start.x + extents, end.x + extents);
    max.y = std::max(start.y + extents, end.x + extents);
    max.z = std::max(start.z + extents, end.x + extents);
}
...
INLINE bool Box::containsPoint(const Vector3 &point) const
{
...
INLINE bool Box::containsPoint2D(const Vector2 &point) const
{
    if (point.x < min.x || point.x > max.x) return false;
    if (point.y < min.y || point.y > max.y) return false;
    return true;
...
INLINE bool Box::containsBox(const Box &box) const
{
...
INLINE bool Box::containsBox2D(const Box &box) const
{
    if (box.min.x < min.x || box.max.x > max.x) return false;
    if (box.min.y < min.y || box.max.y > max.y) return false;
    return true;
...
INLINE bool Box::touchesBox(const Box &other) const
{
...
INLINE bool Box::touchesBox2D(const Box &other) const
{
    if (min.x > other.max.x || max.x < other.min.x) return false;
    if (min.y > other.max.y || max.y < other.min.y) return false;
    return true;
...
INLINE void Box::updateWithPoint(const Vector3 &point)
{
    min = min.min(point);
    max = max.max(point);
}

INLINE void Box::updateWithBox(const Box &box)
{
    min = min.min(box.min);
    max = max.max(box.max);
}
