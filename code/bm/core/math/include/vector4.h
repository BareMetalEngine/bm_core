...
/// 4 component vector
TYPE_ALIGN(16, class) BM_CORE_MATH_API Vector4
{
...
public:
    ...
    INLINE Vector4 minScalar(float val) const;

    // maximum values
    INLINE Vector4 max(const Vector4& other) const;

    // maximum values
    INLINE Vector4 maxScalar(float val) const;

    // clamped to range
    INLINE Vector4 clamped(const Vector4& minV, const Vector4& maxV) const;

    // clamped to range
    INLINE Vector4 clampedScalar(float minV, float maxV) const;

    // clamp in place to range
    INLINE void clamp(const Vector4& minV, const Vector4& maxV);
    ...
    // snap to grid
    INLINE Vector4 snapped(float grid) const;

    // lerp between this vector and target
    INLINE Vector4 lerp(const Vector4& target, float f) const;

    // lerp between this vector and target, per component weight
    INLINE Vector4 lerpPerComponent(const Vector4& target, const Vector4& f) const;
