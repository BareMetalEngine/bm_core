...
#define SIMD_INFINITY bm::VERY_LARGE_FLOAT
...
namespace ole
{
    ...
    private:
        ...
        public:
            ...
            {
                array = (T*)PoolAllocate(bm::MainPool(), sizeof(T) * size, alignof(T));
            }

            ~PoolArray()
            {
                bm::Memory::FreeBlock(array);
            }
        ...
        public:
            ...
            T* newObject()
            {
                ...
                if (!o)
                {
                    ...
                    {
                        auto* ptr = bm::PoolAllocate(bm::MainPool(), sizeof(PoolArray<T>), alignof(PoolArray<T>));
                        p = new (ptr) PoolArray<T>(arraySize);
                        p->next = arrays;
                        arrays = p;
                    }
                    o = p->init();
        ...
        public:
            ...
            T* newObject()
            {
                ...
                if (!o)
                {
                    ...
                    {
                        auto* ptr = PoolAllocate(bm::MainPool(), sizeof(PoolArray<T>), alignof(PoolArray<T>));
                        p = new (ptr) PoolArray<T>(arraySize);
                        p->next = arrays;
                        arrays = p;
                    }
...
static T* AllocPtr(void* base, void*& ptr, uint32_t count=1, uint16_t* outOffset = nullptr)
{
    ...
    // advance
    ptr = OffsetPtr<void>(ptr, count * sizeof(T));
    return writePtr;
}
...
bool Convex::build(const float* points, uint32_t numPoints, uint32_t stride, float shinkBy)
{
    ...
    // allocate memory
    auto data = Buffer::CreateEmpty(MainPool(), memoryNeeded);
