    ...
    static void CopyNode(const QuantizationParams& q, const kd::TreeNode* sourceNodes, const TriMeshBuildTriangle* sourceTriangles, TriMeshNode* destNodes, int nodeIndex, Box& outBox)
    {
        ...
        if (sourceNode.childIndex == -1)
        {
            ...
            for (uint32_t i=0; i<sourceNode.numTriangles; ++i)
            {
                ...
                localBox.updateWithPoint(tri.v0);
                localBox.updateWithPoint(tri.v1);
                localBox.updateWithPoint(tri.v2);
            }
        }
        else
        {
...
void TriMesh::build(TriMeshBuildTriangle* triangles, uint32_t numTriangles)
{
    ...
    for (uint32_t i = 0; i < numTriangles; ++i)
    {
        bounds.updateWithPoint(triangles[i].v0);
        bounds.updateWithPoint(triangles[i].v1);
        bounds.updateWithPoint(triangles[i].v2);
    }
    ...
    // copy nodes
    if (auto nodeData = Buffer::CreateEmpty(MainPool(), sizeof(TriMeshNode) * nodes.size(), 16))
    {
    ...
    if (auto triangleData = Buffer::CreateEmpty(MainPool(), sizeof(TriMeshTriangle) * numTriangles, 16))
    {
