...
class BM_CORE_CONTAINERS_API PagedBuffer : public MainPoolData<NoCopy>
{
public:
    PagedBuffer(uint32_t alignment = 4); // use shared page allocator
    PagedBuffer(IPoolPaged& pool, uint32_t alignment = 4); // use given page allocator
    ~PagedBuffer();
    ...
    void copy(BufferView view) const;
    ...
    Buffer toBuffer(IPoolUnmanaged& pool) const;
...
{
public:
    INLINE PagedBufferTyped(); // use shared page allocator
    INLINE PagedBufferTyped(IPoolPaged& pageAllocator); // use given page allocator
