...
template< typename T >
INLINE StaticStructurePool<T>::StaticStructurePool(const char* tag)
#ifdef PLATFORM_HASTYPEID
    : StaticStructurePoolBase(sizeof(T), alignof(T), tag ? tag : typeid(StaticStructurePool<T>).name())
#else
    : StaticStructurePoolBase(sizeof(T), alignof(T), tag ? tag : "StaticStructurePool")
#endif
{}
