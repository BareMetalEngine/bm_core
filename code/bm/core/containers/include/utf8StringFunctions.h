...
namespace utf8
{
	...
	ALWAYS_INLINE static bool IsUTF8Extension(char c)
	{
		return (c & 0xC0) == 0x80;
	}
    ...
    extern BM_CORE_CONTAINERS_API size_t ToUniChar(wchar_t* dest, size_t sz, const char* src, size_t srcsz);

    // Convert UTF-8 data to the very wide 32-bit character
    // NOTE: the target buffer must be large enough, returns number of written chars
    extern BM_CORE_CONTAINERS_API size_t ToUniChar32(uint32_t* dest, size_t sz, const char* src, size_t srcsz);

    // Convert wide characters to UTF-8 data
    // NOTE: the target buffer must be large enough, returns number of written chars
    extern BM_CORE_CONTAINERS_API size_t FromUniChar(char* dest, size_t sz, const wchar_t* src, size_t srcsz);

    // NOTE: the target buffer must be large enough, returns number of written chars
    extern BM_CORE_CONTAINERS_API size_t FromUniChar32(char* dest, size_t sz, const uint32_t* src, size_t srcsz);

    // Single character to UTF-8, returns number of written chars
    extern BM_CORE_CONTAINERS_API uint8_t ConvertChar(char* dest, uint32_t code);

    // Return next character, updating an index variable
    extern BM_CORE_CONTAINERS_API uint32_t NextChar(const char*& ptr, const char* endPtr);

    // Return true if we can parse a utf-8 character at given position
    extern BM_CORE_CONTAINERS_API bool ValidChar(const char* ptr, const char* endPtr);

    // Get char code parser from utf-8 characters at given position
    extern BM_CORE_CONTAINERS_API uint32_t GetChar(const char* ptr, const char* endPtr);

    // Count the number of characters in a UTF-8 string
	extern BM_CORE_CONTAINERS_API size_t Length(const char* ptr, const char* endPtr);

    // Count memory size required to encode UTF-8 string from uint32_t buffer
    // NOTE: does not include the terminating zero
    extern BM_CORE_CONTAINERS_API size_t CalcSizeRequired(const uint32_t* s, size_t maxLength = MAX_SIZE_T);

    // Count memory size required to encode UTF-8 string from wchar_t buffer
    // NOTE: does not include the terminating zero
    extern BM_CORE_CONTAINERS_API size_t CalcSizeRequired(const wchar_t* s, size_t maxLength = MAX_SIZE_T);
