...
StreamingGrid::~StreamingGrid()
{
    if (m_buckets)
    {
        Memory::FreeBlock(m_buckets);
        m_buckets = nullptr;
    }

    if (m_nodes)
    {
        Memory::FreeBlock(m_nodes);
        m_nodes = nullptr;
    }

    if (m_levels)
    {
        Memory::FreeBlock(m_levels);
        m_levels = nullptr;
    }
}
...
StreamingGridObjectID StreamingGrid::registerObject(uint16_t x, uint16_t y, uint16_t z, uint16_t radius, uint32_t data)
{
    ...
    if (bucketIndex == m_numBuckets)
    {
        ...
        m_buckets = (GridBucket*) PoolReallocate(MainPool(), m_buckets, sizeof(GridBucket) * m_numBuckets, alignof(GridBucket));
    }
...
void StreamingGrid::createBuckets(uint32_t numBuckets)
{
    ...
    m_buckets = (GridBucket*)PoolAllocate(MainPool(), sizeof(GridBucket) * numBuckets, alignof(GridBucket));
...
void StreamingGrid::createGrid(uint32_t numLevels)
{
    ...
    m_nodes = (GridNode*)PoolAllocate(MainPool(), sizeof(GridNode) * m_numNodes, alignof(GridNode));
    memzero(m_nodes, sizeof(GridNode) * m_numNodes);

    // preallocate memory for grid levels
    m_numLevels = numLevels;
	m_levels = (GridLevel*)PoolAllocate(MainPool(), sizeof(GridLevel) * m_numLevels, alignof(GridLevel));
