...
extern BM_CORE_OBJECT_API bool ParsePropertyName(StringView& view, StringView& outPropertyName);

// eat array count from view path, will modify the path to the remainder if successful
extern BM_CORE_OBJECT_API bool ParseArrayIndex(StringView& view, uint32_t& outArrayCount);

// get parent path for given path
extern BM_CORE_OBJECT_API bool ExtractParentPath(StringView& view, StringView& outChild);
