...
class BM_CORE_OBJECT_API SerializationBinaryFileTables
{
public:
    ...
    static const uint16_t FileFlag_ExtractedBuffers = 1;
...
#pragma pack(push, 1)
    ...
    struct Import
    {
        ...
        {
            return (guid[0] == other.guid[0]) && (guid[1] == other.guid[1]) && (guid[2] == other.guid[2]) && (guid[3] == other.guid[3]) && (classTypeIndex == other.classTypeIndex);
        }
    ...
    struct Property
    {
        ...
        {
            return (other.classTypeIndex == classTypeIndex) && (nameIndex == other.nameIndex) && (typeIndex == other.typeIndex);
        }

        static uint32_t CalcHash(const Property& entry)
        {
            return CRC32() << entry.classTypeIndex << entry.nameIndex << entry.typeIndex;
        }
    };
...
#pragma pack(pop)
    ...
    static_assert(sizeof(Export) == 20, "Structure is read directly into memory. Size cannot be changed without an adapter.");
    static_assert(sizeof(Property) == 6, "Structure is read directly into memory. Size cannot be changed without an adapter.");
    ...
    bool validateTables(uint64_t avaiableSize, StringBuf* outError = nullptr) const;

    // check if the header is valid
    bool validateHeader(uint64_t avaiableSize) const;
    ...
    static bool ValidateHeader(const Header& header, uint32_t sizeOfData, StringBuf* outError=nullptr);
