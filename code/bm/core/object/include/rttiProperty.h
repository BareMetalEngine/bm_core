...
enum class PropertyFlagBit : uint32_t
{
    ...
    ScriptVisible = FLAG(10), // property is explicitly made visible to scripting engine, only those properties can be imported
...
struct BM_CORE_OBJECT_API PropertyEditorData
{
...
class BM_CORE_OBJECT_API Property : public MetadataContainer
{
public:
    ...
    ALWAYS_INLINE const void* offsetPtr(const void* data) const
    {
        const void* root = externalBuffer() ? *(const void**)((const char*)data + OBJECT_OFFSET_TO_SCRIPT_DATA) : data;
        return OffsetPtr<void>(root, offset());
    }

    /// get property data given the pointer to the object
    ALWAYS_INLINE void* offsetPtr(void* data) const
    {
        void* root = externalBuffer() ? *(void**)((char*)data + OBJECT_OFFSET_TO_SCRIPT_DATA) : data;
        return OffsetPtr<void>(root, offset());
    }
