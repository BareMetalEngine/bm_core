...
class BM_CORE_OBJECT_API IAsyncFileBufferLoader : public IReferencable
{
public:
    ...
    virtual bool extract(TaskContext& tc, IPoolUnmanaged& pool, Buffer& outCompressedData, CompressionType& outCompressionType) const = 0;

    // peek at existing data (uncompressed only)
    virtual Buffer peak() const { return nullptr; }
    ...
    virtual Buffer load(TaskContext& tc, IPoolUnmanaged& pool, uint32_t alignment = DEFAULT_BUFFER_ALIGNMENT) const = 0;

    //--

    /*// load content of the buffer (not cached, each call will start new loading)
    Buffer load(TaskContext& tc, IPoolUnmanaged& pool, )
    {
        auto ret = Buffer::CreateEmpty(pool, size(), alignment);
        DEBUG_CHECK_RETURN_EX_V(ret, "Out of memory allocating buffer", nullptr);
        DEBUG_CHECK_RETURN_EX_V(load(tc, ret), "Failed to load buffer content", nullptr);
        return ret;
    }*/
    ...
    // create resident buffer from already compressed data, NOTE: the buffer refcount will be increased
...
class BM_CORE_OBJECT_API AsyncFileBuffer
{
public:
    AsyncFileBuffer();
    ...
    void setup(Buffer uncompressedData, CompressionType compression = CompressionType::Uncompressed);
    ...
    Buffer load(TaskContext& tc, IPoolUnmanaged& pool, uint32_t alignment = 16) const;
    ...
    //CAN_YIELD bool loadInto(TaskContext& tc, BufferView ptr) const;
