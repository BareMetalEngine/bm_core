...
struct BM_CORE_OBJECT_API GlobalEventKey
{
public:
...
private:
    ...
    friend BM_CORE_OBJECT_API GlobalEventKey MakeUniqueEventKey(StringView);
    friend BM_CORE_OBJECT_API GlobalEventKey MakeSharedEventKey(StringView);
};

//--

// allocate unique event key, good for tracking object instances, etc
extern BM_CORE_OBJECT_API GlobalEventKey MakeUniqueEventKey(StringView debugInfo = "");

// allocate shared event key - will return same value for the same path, great for files
extern BM_CORE_OBJECT_API GlobalEventKey MakeSharedEventKey(StringView path);
