...
void ConfigStorage::Append(ConfigStorage& target, const ConfigStorage& from)
{
    for (const auto* sourceGroup : from.m_groups.values())
    {
        auto& copyGroup = target.group(sourceGroup->name());
        for (const auto* sourceEntry : sourceGroup->entries())
        {
            auto& copyEntry = copyGroup.entry(sourceEntry->name());
            for (const auto& val : sourceEntry->values())
                copyEntry.appendValue(val);
        }
    }
}
