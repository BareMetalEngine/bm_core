...
void IHandleType::writeBinary(TypeSerializationContext& typeContext, SerializationWriter& file, const void* data, const void* defaultData) const
{
    ...
    file.writePointer(handle, isStrongHandle());
}
...
void IHandleType::writeText(TypeSerializationContext& typeContext, ITextSerializationWriter& file, const void* data, const void* defaultData) const
{
	ObjectPtr object;
	readPointedObject(data, object);
    file.writeValueObject(object, isStrongHandle());
}

void IHandleType::readText(TypeSerializationContext& typeContext, ITextSerializationReader& file, void* data) const
{
	ObjectPtr object;
    file.readValueObject(object);

    if (object && !object->is(m_pointedType))
    {
        file.reportError(TempString("Object '{}' is not compatible with pointer type '{}' at {}", object->cls(), m_pointedType, typeContext));
        object = nullptr;
    }
...
#if 0
void IHandleType::writeXML(TypeSerializationContext& typeContext, XMLNodeHandle& node, const void* data, const void* defaultData) const
{
    ObjectPtr object;
    readPointedObject(data, object);
...
void IHandleType::readXML(TypeSerializationContext& typeContext, const XMLNodeHandle& node, void* data) const
{
    ObjectPtr object;
