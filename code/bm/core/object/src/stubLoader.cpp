...
#include "bm/core/containers/include/crc.h"
...
StubLoader::StubLoader(const StubFactory& factory, uint32_t minVersion, const char* tag)
	: m_factory(factory)
...
void StubLoader::clear()
{
	...
	Memory::FreeBlock(m_stubMem);
	m_stubMem = nullptr;

	m_unpackedMem = Buffer();
}

struct StubLoaderCleanupHelper : public MainPoolData<NoCopy>
{
public:
...
const IStub* StubLoader::unpack(const void* packedData, uint32_t packedDataSize)
{
	...
	const auto packedDataView = BufferView(header + 1, packedDataSize - headerSize);
	auto unpackedMem = Buffer::CreateDecompressed(MainPool(), CompressionType::LZ4HC, packedDataView, header->uncompressedSize);
	...
	auto* objectMemoryBlock = (uint8_t*)Memory::AllocateBlock(totalNeededMemory + header->additionalDataSize, totalNeededAlignment, m_tag);
