...
namespace prv
{
    ...
    {
        StringBuilder txt;
        IType::PrintToString(srcType, srcData, txt);
    ...
    static bool FromStringConversion(const void* srcData, const IType* srcType, void* destData, const IType* destType)
    {
        ...
        return IType::ParseFromString(destType, destData, src.view());
    }
...
/*            static bool Conversion_SyncHandleToAsyncHandle(const void* srcData, const IType* srcType, void* destData, const IType* destType)
    ...
    {
        DEBUG_CHECK(srcType->metaType() == MetaType::ResourceRef);
        DEBUG_CHECK(destType->metaType() == MetaType::ResourceRef);
    ...
    public:
        ConversionMatrix()
        {
            ...
            SET_CAST_RAW(ResRef, ResRef, Conversion_ResourceHandle);
...
bool ConvertData(const TypedMemory& src, const TypedMemory& dest)
{
    return ConvertData(src.rawData(), src.rawType(), (void*)dest.rawData(), dest.rawType());
}
