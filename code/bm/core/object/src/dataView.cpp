...
const char* CodeName(DataViewResultCode code)
{
    switch (code)
    {
        ...
        case DataViewResultCode::ErrorIncompatibleMultiView: return "ErrorIncompatibleMultiView";
    }
...
IDataView::IDataView()
{
    const auto rootPathHash = StringView("").evaluateCRC64();
    m_rootPath = new Path;
...
{
    const auto pathCode = fullPath.evaluateCRC64();
...
{
    const auto pathCode = path.evaluateCRC64();
