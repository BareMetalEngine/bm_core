...
namespace prv
{
    ...
    public:
        ...
        {
            if (loc.contextName())
                Log::Stream(LogOutputLevel::Error, loc.contextName().c_str(), loc.line()).append(message.data(), message.length());
            else
...
Token* TextParser::parseToken(LocalAllocator& mem, const ITextLanguageDefinition& language)
{
    auto curLineIndex = m_lineIndex;

    // parse
    auto cur  = m_pos;
    auto token  = mem.createWithoutDestruction<Token>();
...
bool TextParser::findDjangoTag(StringView& outSkipped, StringView& outTag, char& outTagType, TextTokenLocation& outLocation)
{
	...
	while (m_pos < m_end)
	{
        if (m_pos[0] == '{') // look for {{ {# {%
        {
            // is this a proper start ?
            char endTagChar = 0;
            auto startLine = m_lineIndex;
            const char nch = (m_pos + 1 < m_end) ? m_pos[1] : 0;
            switch (nch)
            {
                case '{':
                    outSkipped = StringView(start, m_pos);
                    outLocation = TextTokenLocation(m_context, m_lineIndex);
					outTagType = nch;
                    endTagChar = '}';
                    m_pos += 2;
                    break;
            ...
            {
                // look for end
                const auto* tagStart = m_pos;
                while (m_pos < m_end)
                {
                    if (m_pos[0] == endTagChar) // look for }} #} or %}
                    {
                        const char nch = (m_pos + 1 < m_end) ? m_pos[1] : 0;
                        if (nch == '}')
                        {
                            outTag = StringView(tagStart, m_pos).trim();
                            m_pos += 2;
                            return true;
                        }
                    }
                ...
                // not found
                return error(TempString("Unexpected end of file before closing tag at line {} was found", startLine));
			}
        }
    ...
    // not found
    outSkipped = StringView(start, m_pos); // all skipped
    return false;
}
