...
namespace prv
{
    ...
    ParsingTreeBuilder::ParsingTreeBuilder()
    {
        ...
        m_root = m_mem.createWithoutDestruction<ParsingNode>();
        m_root->m_index = 0;
    ...
    {
        auto newNode  = (node == nullptr) ? m_mem.createWithoutDestruction<ParsingNode>() : m_mem.createWithoutDestruction<ParsingNode>(*node);
    ...
    ParsingNode* ParsingTreeBuilder::uniqueMatchNode(ParsingNode* atNode, const char* chars)
    {
        ...
        for (auto ch  = chars; *ch; ++ch)
        {
            ...
            if (atNode->m_continuation[(int)*ch].m_node == nullptr)
                return nullptr;

            // make sure it's the same node for all chars
            if (!followerNode)
                followerNode = atNode->m_continuation[(int)*ch].m_node;
            else if (atNode->m_continuation[(int)*ch].m_node != followerNode)
                return nullptr;
    ...
    void ParsingTreeBuilder::insertUniqueMatchNode(ParsingNode* atNode, const char* chars, Array<ParsingNode*>& outContinuationNodes)
    {
        ...
        {
            // we don't have a follower node at this char
            auto continuationNode  = atNode->m_continuation[(int)*ch].m_node;
        ...
        for (auto ch  = chars; *ch; ++ch)
        {
            ...
            ASSERT(nodeIndex != -1);
            atNode->m_continuation[(int)*ch].m_node = outContinuationNodes[nodeIndex];
        }
    }
    ...
    {
        // get current match
        auto currentMatchNode  = atNode->m_continuation[(int)ch].m_node;
        ...
        atNode->m_continuation[(int)ch].m_node = newMatchNode;
        return newMatchNode;
    }
    ...
    {
        while (*spinChars)
            node->m_continuation[(int)*spinChars++].m_node = node;
    }

    void ParsingTreeBuilder::spinOnSelf(ParsingNode* node, char spinChar)
    {
        node->m_continuation[(int)spinChar].m_node = node;
    }
    ...
    /*void ParsingTreeBuilder::insertIntegerMatch()
    {
        ...
        InplaceArray<ParsingNode*, 64> firstNodes;
        insertUniqueMatchNode(m_root, ALLOWED_CHARS, firstNodes);

        // make the number spin on itself as long as we have digits
        for (auto firstNode : firstNodes)
        {
            spinOnSelf(firstNode, ALLOWED_CHARS);
    ...
    void ParsingTreeBuilder::insertHexadecimalsMatch()
    {
        // then we just follow the typical hex chars
        const char* ALLOWED_CHARS = "0123456789abcdefABCDEF";

        // hexadecimal starts with 0x combination
        auto head = insertUniqueMatchNode(m_root, '0');
        head = insertUniqueMatchNode(head, 'x');

        // match the digits after the header
        InplaceArray<ParsingNode*, 64> firstNodes;
        insertUniqueMatchNode(head, ALLOWED_CHARS, firstNodes);

        // continue matching
        // NOTE: we only have a conditional exit here (possibleMatch) since it can go on to infinity
        for (auto firstNode : firstNodes)
        {
            spinOnSelf(firstNode, ALLOWED_CHARS);
            firstNode->m_possibleEmit = TextTokenType::Hex;
        }
    }

    void ParsingTreeBuilder::insertFloatingExponentValueEnding(ParsingNode* node)
    {
        const char* ALLOWED_CHARS = "0123456789";

        // match the digits at root
        InplaceArray<ParsingNode*, 64> firstNodes;
        insertUniqueMatchNode(node, ALLOWED_CHARS, firstNodes);

        // make the number spin on itself as long as we have exponent
        for (auto firstNode : firstNodes)
        {
            spinOnSelf(firstNode, ALLOWED_CHARS);
            firstNode->m_possibleEmit = TextTokenType::FloatNumber;
        }
    }

    void ParsingTreeBuilder::insertFloatingExponentEnding(ParsingNode* node)
    {
        // we may get a '+'
        auto plusNode = insertUniqueMatchNode(node, '+');
        insertFloatingExponentValueEnding(plusNode);

        // or minus 
        auto minusNode = insertUniqueMatchNode(node, '-');
        insertFloatingExponentValueEnding(minusNode);

        // or nothing
        insertFloatingExponentValueEnding(node);
    }

    void ParsingTreeBuilder::insertFloatingPointEnding(ParsingNode* rootNode)
    {
        // we may end with 'f'
        {
            auto finalNode = insertUniqueMatchNode(rootNode, 'f');
            makeFinalNode(finalNode);
        ...
        // or we may have the 'e' or 'E' scientific notation
        {
            auto finalNode = insertUniqueMatchNode(rootNode, 'e');
            insertFloatingExponentEnding(finalNode);
        }
    ...
    void ParsingTreeBuilder::insertFloatingFractionPart(ParsingNode* root, bool hasNumberBefore)
    {
        const char* ALLOWED_CHARS = "0123456789";

        // always expect a point
        auto dot = insertUniqueMatchNode(root, '.');
        if (hasNumberBefore)
            dot->m_possibleEmit = TextTokenType::FloatNumber;

        // we may exit here
        if (hasNumberBefore)
            insertFloatingPointEnding(dot);

        // dot can be followed by any digit
        InplaceArray<ParsingNode*, 64> firstNodes;
        insertUniqueMatchNode(dot, ALLOWED_CHARS, firstNodes);

        // each digit can go into any other as long as they want to and it's still a valid number
        // ie. .1 is valid, so it .12124124
        for (auto firstNode : firstNodes)
        {
            // make ourselves spin on the numbers
            firstNode->m_possibleEmit = TextTokenType::FloatNumber;
            spinOnSelf(firstNode, ALLOWED_CHARS);

            // at any point we may end
            insertFloatingPointEnding(firstNode);
        }
    }

    void ParsingTreeBuilder::insertFloatingPointMatchFull(ParsingNode* root)
    {
		const char* ALLOWED_CHARS = "0123456789";
		...
		// allow no decimal part
		insertFloatingFractionPart(root, false);

		// match the digits at root
		InplaceArray<ParsingNode*, 64> firstNodes;
		insertUniqueMatchNode(root, ALLOWED_CHARS, firstNodes);
		...
		for (auto firstNode : firstNodes)
		{
			...
			// NOTE: also, if we ended NOW it would actually be integer since there was no dot yet
			firstNode->m_possibleEmit = TextTokenType::IntNumber;
			spinOnSelf(firstNode, ALLOWED_CHARS);

			// create a node that follows the '.'
			insertFloatingFractionPart(firstNode, true);

			// or we may have the 'e' or 'E' scientific notation
			{
				auto finalNode = insertUniqueMatchNode(firstNode, 'e');
				insertFloatingExponentEnding(finalNode);
			}
			...
			{
				auto finalNode = insertUniqueMatchNode(firstNode, 'f');
				makeFinalNode(finalNode);
				ASSERT_EX(finalNode->m_possibleEmit == TextTokenType::Invalid, "Final node already constructed");
				finalNode->m_possibleEmit = TextTokenType::FloatNumber;
			}
			...
			{
				auto finalNode = insertUniqueMatchNode(firstNode, 'u');
				makeFinalNode(finalNode);
				ASSERT_EX(finalNode->m_possibleEmit == TextTokenType::Invalid, "Final node already constructed");
				finalNode->m_possibleEmit = TextTokenType::IntNumber;
			}
			...
			{
				auto finalNode = insertUniqueMatchNode(firstNode, 'i');
				makeFinalNode(finalNode);
				ASSERT_EX(finalNode->m_possibleEmit == TextTokenType::Invalid, "Final node already constructed");
				finalNode->m_possibleEmit = TextTokenType::IntNumber;
			}
    ...
    {
		/*// we may get a '+'
		auto plusNode = insertUniqueMatchNode(m_root, '+');
        insertFloatingPointMatchFull(plusNode);

		// or minus 
		auto minusNode = insertUniqueMatchNode(m_root, '-');
        insertFloatingPointMatchFull(minusNode);*/

		// or nothing
        insertFloatingPointMatchFull(m_root);
    }
    ...
    void ParsingTreeBuilder::print(IFormatStream& str) const
    {
        ...
        for (uint32_t i=0; i<m_nodes.size(); ++i)
        {
            ...
            switch (node->m_possibleEmit)
            {
                ...
                case TextTokenType::Preprocessor:
                    str.append("emit: Preprocessor");
                    break;
