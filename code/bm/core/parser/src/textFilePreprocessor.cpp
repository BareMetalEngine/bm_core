...
static UniquePtr<ITextLanguageDefinition> BuildPreprocessorLanguageDefinition()
{
    ...
    langBuilder.addPreprocessor("#error");
    langBuilder.addPreprocessor("#message");
    langBuilder.enableStrings(true);
    langBuilder.enableFloatNumbers(true);
    langBuilder.enableHexadecimalNumbers(true);
...
TextFilePreprocessor::TextFilePreprocessor(LocalAllocator& allocator, ITextIncludeHandler& includeHandler, ITextErrorReporter& errorHandler, ITextCommentEater& commentEater, const ITextLanguageDefinition& parentLanguage)
...
static bool GTracePreprocessor = false;
...
{
    auto ret  = m_allocator.createWithoutDestruction<Token>(*source);
...
bool TextFilePreprocessor::createTokens(const Token* baseLocataion, StringView text, TokenList& outList)
{
    ...
    while (ptr < end)
    {
        ...
        auto token  = m_allocator.createWithoutDestruction<Token>();
...
bool TextFilePreprocessor::processFileContent(StringView content, StringView contextPath, TokenList& outTokenList)
{
    ...
    while (parser.findNextContent())
    {
        ...
        else
        {
            ...
            auto lookAhead = parser.view().leftPart(20).trim()
                .beforeFirst(" ", StringCaseComparisonMode::WithCase, StringFindFallbackMode::Full)
                .beforeFirst("\n", StringCaseComparisonMode::WithCase, StringFindFallbackMode::Full)
                .beforeFirst("\r", StringCaseComparisonMode::WithCase, StringFindFallbackMode::Full)
                .beforeFirst("\t", StringCaseComparisonMode::WithCase, StringFindFallbackMode::Full);
...
bool TextFilePreprocessor::evalParentFilterFlag() const
{
    auto size = m_filterStack.size();
    for (uint32_t i = 1; i < size; ++i)
        if (!m_filterStack[i - 1].m_enabled)
            return false;

    return true;
}

bool TextFilePreprocessor::evalFilterFlag() const
{
    for (const auto& elem : m_filterStack)
        if (!elem.m_enabled)
            return false;
...
bool TextFilePreprocessor::processDefine(const Token* head, TokenList& line)
{
    ...
    if (!line.empty())
    {
        ...
        if (GTracePreprocessor)
            m_errorHandler.reportWarning(head->location(), TempString("Macro '{}' defined as '{}'", ident, macro->m_replacement));
    }
...
bool TextFilePreprocessor::processIf(const Token* head, TokenList& line)
{
    if (!evalFilterFlag())
    {
        ...
        info.m_anyTaken = false;
        if (GTracePreprocessor)
            m_errorHandler.reportWarning(head->location(), TempString("pushed stack {}->{}, disabled", m_filterStack.size() - 1, m_filterStack.size()));
        return true;
    }
    ...
    if (GTracePreprocessor)
        m_errorHandler.reportWarning(head->location(), TempString("pushed stack {}->{}, result: {}", m_filterStack.size() - 1, m_filterStack.size(), result));
...
bool TextFilePreprocessor::processIfdef(const Token* head, TokenList& line, bool swap)
{
    ...
    if (GTracePreprocessor)
    {
        m_errorHandler.reportWarning(head->location(), TempString("evaluating {} of {}: {}",
            swap ? "ifndef" : "ifdef", identToken->view(), valid));
    }
    ...
    if (GTracePreprocessor)
    {
        m_errorHandler.reportWarning(head->location(), TempString("pushed stack {}->{}, result: {}",
            m_filterStack.size() - 1, m_filterStack.size(), m_filterStack.back().m_enabled));
    }
...
bool TextFilePreprocessor::processElif(const Token* head, TokenList& line)
{
    ...
    if (!evalParentFilterFlag())
        return true;

    if (filter.m_anyTaken)
    {
        filter.m_token = head;
...
bool TextFilePreprocessor::processEndif(const Token* head, TokenList& line)
{
    ...
    if (GTracePreprocessor)
        m_errorHandler.reportWarning(head->location(), TempString("popped stack {}->{}", m_filterStack.size(), m_filterStack.size() - 1));
...
bool TextFilePreprocessor::processError(const Token* head, TokenList& line)
{
    if (line.empty())
    {
        m_errorHandler.reportError(head->location(), "empty #error directive");
        return false;
    }

    auto pragmaType = line.popFront()->view();
    m_errorHandler.reportError(head->location(), pragmaType);

    return true;
}

bool TextFilePreprocessor::processMessage(const Token* head, TokenList& line)
{
    if (line.empty())
    {
        m_errorHandler.reportError(head->location(), "empty #message directive");
        return false;
    }

    auto pragmaType = line.popFront()->view();
    m_errorHandler.reportWarning(head->location(), pragmaType);
...
bool TextFilePreprocessor::processPreprocessorDeclaration(TokenList& line, bool& keepProcessing)
{
    ...
    const auto filter = evalFilterFlag();

    if (type->view() == "#define")
        return !filter || processDefine(type, line);
    else if (type->view() == "#include")
        return !filter || processInclude(type, line);
    else if (type->view() == "#undef")
        return !filter || processUndef(type, line);
        ...
        return !filter || processPragma(type, line, keepProcessing);
    else if (type->view() == "#error")
        return !filter || processError(type, line);
    else if (type->view() == "#message")
        return !filter || processMessage(type, line);
    else if (filter)
    {
...
TextFilePreprocessor::MacroDefinition* TextFilePreprocessor::createDefine(StringView name)
{
    ...
    {
        ret = m_allocator.createWithoutDestruction<MacroDefinition>();
...
{
    DEBUG_CHECK_RETURN_EX_V(!list.empty(), "Empty token list", false);

    auto definedFunc = [this](StringView name) -> bool {
        return m_defineMap.contains(name);
    };

    TextFilePreprocessorExpressionContext parser(list, m_errorHandler, definedFunc);
...
bool TextFilePreprocessor::expandAllPossibleMacros(TokenList& list, TokenList& outputList, bool fromParamsOnly)
{
    ...
    while (list.head())
    {
        ...
        if (list.head()->view() == "defined")
        {
            auto cur = list.popFront();

            auto bracket = list.popFront();
            if (!bracket || bracket->view() != "(")
            {
                m_errorHandler.reportError(cur->location(), TempString("expected '(' after defined"));
                return false;
            }

            auto ident = list.popFront();
            if (!ident || ident->type() != TextTokenType::Identifier)
            {
                m_errorHandler.reportError(cur->location(), TempString("expected identifier after '('"));
                return false;
            }

            auto rbracket = list.popFront();
            if (!rbracket || rbracket->view() != ")")
            {
                m_errorHandler.reportError(cur->location(), TempString("expected ')' after identifier"));
                return false;
            }

            const char* text = m_defineMap.contains(ident->view()) ? "1" : "0";

            auto* token = m_allocator.createWithoutDestruction<Token>(TextTokenType::IntNumber, text, text+1, -1);
            outputList.pushBack(token);
        }
        else if (shouldExpandMacro(list, macro, fromParamsOnly))
