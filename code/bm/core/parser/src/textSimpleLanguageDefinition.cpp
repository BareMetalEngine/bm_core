...
void SimpleLanguageDefinitionBuilder::enableHexadecimalNumbers(bool enabled)
{
    m_enabledHexadecimals = enabled;
}
...
UniquePtr<ITextLanguageDefinition> SimpleLanguageDefinitionBuilder::buildLanguageDefinition() const
{
    ...
    if (m_enabledHexadecimals) tree.insertHexadecimalsMatch();
    ...
    // dump the structure
    //Log::Stream(LogOutputLevel::Info) << tree;
