...
class BM_CORE_PARSER_API TextParser : public MainPoolData<NoCopy>
{
public:
    ...
    INLINE const char* pos() const { return m_pos; }
    ...
    // skip white spaces till something is found, will close line boundaries and skip comments via the comment eater
    ...
    Token* parseToken(LocalAllocator& mem, const ITextLanguageDefinition& language);
    ...
    // discard history state without popping
    void discardState();

    //--

    // advance to location starting from {{ {# or {%
    bool findDjangoTag(StringView& outSkipped, StringView& outTag, char& outTagType, TextTokenLocation& outLocation);
